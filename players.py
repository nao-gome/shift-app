...
with st.sidebar:
    ...
    if st.session_state.user_role == "admin" and not df_players.empty:
        ...
        # === 追加部分: 選手削除機能 ===
        st.markdown("---")
        with st.expander("💀 選手登録の削除"):
            st.warning(f"⚠️ {edit_target} を完全に削除しますか？\n(これまでの体調データ、テスト記録も全て消えます)")
            if st.checkbox("削除を確認する", key="del_confirm"):
                if st.button("削除を実行", type="primary"):
                    ...
                    # 関連データも削除（ゴミを残さない）
                    df_cond = df_cond[df_cond["名前"] != edit_target]
                    df_cond.to_csv(CONDITION_FILE, index=False, encoding="utf-8-sig")
                    
                    df_phys = df_phys[df_phys["名前"] != edit_target]
                    df_phys.to_csv(PHYSICAL_FILE, index=False, encoding="utf-8-sig")
                    
                    st.success(f"{edit_target} を削除しました。")
                    st.session_state.selected_player_name = None # 選択状態をリセット
                    st.rerun()
...
if st.session_state.user_role == "admin":
    ...
    with t2:
        if st.session_state.selected_player_name and not df_players.empty:
        ...
        else: st.info("選手名簿から選手を選択するか、選手が登録されていません")
    ...
    # 選手ビュー
    if not df_players.empty:
        my_info_list = df_players[df_players["名前"] == st.session_state.user_name]
        if not my_info_list.empty:
            my_info = my_info_list.iloc[0]
